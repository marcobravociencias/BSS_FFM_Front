...
/*
 * Given a jQuery <tr> set, returns the <td>'s that do not have multi-line
 * rowspans. Would use the [rowspan] selector, but never not defined in IE8.
 */
...
/*
 * A Scroller with additional functionality: - optional ScrollerCanvas for
 * content - fired events for scroll start/end - cross-browser normalization of
 * horizontal scroll for RTL
 */
...
EnhancedScroller = (function(superClass) {
  ...
  /*
	 * Always returns the number of pixels scrolled from the leftmost position
	 * (even if RTL). Always positive.
	 */
...
/*
 * A Scroller, but with a wrapping div that allows "clipping" away of native
 * scrollbars, giving the appearance that there are no scrollbars.
 */
...
ClippedScroller = (function(superClass) {
  ...
  /*
	 * Received overflows can be set to 'clipped', meaning scrollbars shouldn't
	 * be visible to the user, but the area should still scroll.
	 */
...
/*
 * A rectangular area of content that lives within a Scroller. Can have
 * "gutters", areas of dead spacing around the perimeter. Also very useful for
 * forcing a width, which a Scroller cannot do alone. Has a content area that
 * lives above a background area.
 */
...
ResourceManager = (function(superClass) {
  ...
  /*
	 * Will always fetch, even if done previously. Accepts optional
	 * chrono-related params to pass on to the raw resource sources. Returns a
	 * promise.
	 */
  ...
  /*
	 * Accepts optional chrono-related params to pass on to the raw resource
	 * sources. Calls callback when done.
	 */
...
ResourceViewMixin = {
  ...
  /*
	 * Hacks
	 * ------------------------------------------------------------------------------------------------------------------
	 * These triggers usually call mutateEvent with dropLocation, which causes
	 * an event modification and rerender. But mutateEvent isn't aware of
	 * eventResourceField, so it might be setting the wrong property.
	 * Workaround. TODO: normalize somewhere else. maybe make a hook in core.
	 */
...
/*
 * For vertical resource view. For views that rely on grids that render their
 * resources and dates in the same pass.
 */
...
/*
 * Requirements: - must be a Grid - grid must have a view that's a ResourceView -
...
TimelineGrid = (function(superClass) {
  ...
  /*
	 * Makes the given date consistent with isTimeScale/largeUnit, so, either
	 * removes the times, ensures a time, or makes it the startOf largeUnit.
	 * Strips all timezones. Returns new copy. TODO: should maybe be called
	 * "normalizeRangeDate".
	 */
  ...
  TimelineGrid.prototype.fgSegHtml = function(seg, disableResizing) {
    ...
    content += '<i class="fa fa-circle '+event.semaforo+'" aria-hidden="true"></i>&nbsp;&nbsp;';
    content += event.os+ '&nbsp;&nbsp;<small><strong>' + event.intervecion + '</strong></small>';
    content += '</div>';
    content += '<div class="row"  style="background-color: #fff; border-top-left-radius: 5px; border-top-right-radius: 5px;" onmouseover="this.style.background=\''+event.hover+'\';" onmouseout="this.style.background=\'#fff\';">';
    content += '<div class="container">';
    content += '<div class="row" >';
    content += ' &nbsp; &nbsp;'+event.title;
    content += '</div>';
    content += '<div class="row">';
    content += ' &nbsp; &nbsp;'+(event.telefono ? event.telefono : 'S/T');
    if(event.propietarioot == 1){
    	content += '<img src="'+finalphat+'/img/unidad/tpe2.png" style="position: absolute; z-index: 1;top: 70%;right: 2%;">';
    }else{
    	content += '<img src="'+finalphat+'/img/unidad/solucion2.png" style="position: absolute; z-index: 1;top: 70%;right: 2%;">';
    }
    content += '</div>';
    content += '<div class="row">';
//    	if(event.propietario != 1  ){
//    		var chek = event.descargacheck;
//    		
//    		content += ' &nbsp; &nbsp;<i class="fa fa-download" aria-hidden="true" onclick="donwloadchecklist(\''+chek+'\')"></i>';
//  		}
    content += ' &nbsp; &nbsp;<i class="fa fa-id-card-o" aria-hidden="true" onclick="modalDetalleOTOperario(\''+event.title+'\', \''+event.id+'\', \''+event.resourceId+'\', \''+event.idstatus+'\', \''+event.tipo_ot+'\')"></i>';
    content += ' &nbsp; &nbsp;<i class="fa fa-commenting" aria-hidden="true" onclick="modalComentarios(\''+event.title+'\', \''+event.id+'\')"></i>';
    content += ' &nbsp; &nbsp;<i class="fa fa-header" aria-hidden="true" onclick="modalHistorico(\''+event.title+'\', \''+event.id+'\')"></i>';
    content += '</div>';
    content += '</div>';
    content += '</div>';
    content += '</div>';
    
    return '<a class="' + classes.join(' ') + '" style=" background-color: '+ event.color + '; margin-top: 3px !important;"' + (event.url ? ' href="' + htmlEscape(event.url) + '"' : '') + '  >' + '<div class="fc-content">' + (timeText ? '<span class="fc-time">' + htmlEscape(timeText) + '</span>' : '') + '<div class="fc-title"> ' + content  + '</div>' + '</div>' + '<div class="fc-bg" />' + (isResizableFromStart ? '<div class="fc-resizer fc-start-resizer"></div>' : '') + (isResizableFromEnd ? '<div class="fc-resizer fc-end-resizer"></div>' : '') + '</a>';
// return '<a class="' + classes.join(' ') + '" style="' +
// cssToStr(this.getSegSkinCss(seg)) + '"' + (event.url ? ' href="' +
// htmlEscape(event.url) + '"' : '') + '>' + '<div class="fc-content">' +
// (timeText ? '<span class="fc-time">' + htmlEscape(timeText) + '</span>' : '')
// + '<span class="fc-title"> ' + (event.title ? htmlEscape(event.title) :
...
TimelineGrid.prototype.initScaleProps = function() {
  ...
  /*
	 ...
	 * console.log('isTimeScale', @isTimeScale) console.log('largeUnit',
	 * @largeUnit)
	 */
...
ResourceTimelineView = (function(superClass) {
  ...
  /*
	 * TODO: the scenario where there were previously unassociated events that
	 * are now attached to this resource. should render those events
	 * immediately.
	 * 
...
ResourceTimelineGrid = (function(superClass) {
  ...
  /*
	 * Called when a row has been added to the tree data structure, but before
	 * it's rendered. Computes and assigns business hour data *if necessary*. To
	 * be rendered soon after.
	 */
  ...
  /*
	 ...
	 * Unrenders the row's segs and, if necessary, forces businessHours back to
	 * generic rendering.
	 */
...
Spreadsheet = (function() {
  ...
  Spreadsheet.prototype.renderHeadHtml = function() {
    ...
    for (i = k = 0, len1 = colSpecs.length; k < len1; i = ++k) {
      ...
      html += '<th class="' + this.view.widgetHeaderClass + '">' + '<div>' + '<div class="fc-cell-content">' + (o.isMain ? '<span class="fc-expander-space">' + '<span class="fc-icon"></span>' + '</span>' : '') + '<span class="fc-cell-text" data-step="24" data-intro="<strong>Operarios</strong><img style=\'width: 100%;\' src=\'/FFM_TPE/img/introjs/instalador.PNG\'><table  border=\'1\'><tr><th><small>Estado</small></th><th><small>informaci&oacute;n</small></th><th><small>Ubicaci&oacute;n</small></th></tr><tr><td><img src=\'/FFM_TPE/img/introjs/estadooper.PNG\'></td><td><img  src=\'/FFM_TPE/img/introjs/infoot.PNG\'></td><td><img src=\'/FFM_TPE/img/introjs/ubicacion.PNG\'></td></tr></table>">' + htmlEscape(o.labelText || '') + '</span>' + '</div>' + (!isLast ? '<div class="fc-col-resizer"></div>' : '') + '</div>' + '</th>';
    }
    html += '</tr>';
    html += '</tbody></table>';
    return html;
...
/*
 * An abstract node in a row-hierarchy tree. May be a self-contained single row,
 * a row with subrows, OR a grouping of rows without its own distinct row.
 */
...
RowParent = (function() {
  ...
  /*
	 * Adds the given node as a child. Will be inserted at the `index`. If not
	 * given, will be appended to the end.
	 */
  ...
  /*
	 * Removes the given child from the node. Assumes it is a direct child. If
	 * not a direct child, returns false and nothing happens. Unrenders the
	 * child and triggers handlers.
	 */
  ...
  /*
	 * Removes all of the node's children from the hierarchy. Unrenders them and
	 * triggers callbacks. NOTE: batchRows/unbatchRows should probably be called
	 * before this happens :(
	 */
  ...
  /*
	 * Walks backward in the hierarchy to find the previous row leaf node. When
	 * looking at the hierarchy in a flat linear fashion, this is the revealed
	 * row just before the current.
	 */
  ...
  /*
	 * Generates a flat array containing all the row-nodes of the subtree.
	 * Descendants + self
	 */
  ...
  /*
	 * Generates a flat array containing all the nodes (row/non-row) of the
  ...
  /*
	 * Generates a flat array containing all the descendant nodes the current
	 * node
	 */
  ...
  /*
	 * Builds and populates the TRs for each row type. Inserts them into the
	 * DOM. Does this only for this single row. Not recursive. If not a row
	 * (hasOwnRow=false), does not render anything. PRECONDITION: assumes the
	 * parent has already been rendered.
	 */
  ...
  /*
	 * Unpopulates and removes all of this row's TRs from the DOM. Only for this
	 * single row. Not recursive. Will trigger "hidden".
	 */
  ...
  /*
	 * Like unrender(), but does it for this row AND all descendants. NOTE:
	 * batchRows/unbatchRows should probably be called before this happens :(
	 */
  ...
  /*
	 * Renders this row if not already rendered, making sure it is visible. Also
	 * renders descendants of this subtree, based on whether they are expanded
	 * or not. NOTE: If called externally, batchRows/unbatchRows should probably
  ...
  /*
	 * Temporarily hides this node's TRs (if applicable) as well as all nodes in
	 * the subtree
	 */
  ...
  /*
	 * Reveals this node's children if they have not already been revealed.
	 * Changes any expander icon.
	 */
  ...
  /*
	 * Hides this node's children if they are not already hidden. Changes any
	 * expander icon.
	 */
  ...
  /*
	 * Causes a slide-down CSS transition to demonstrate that the expand has
	 * happened
	 */
  ...
  /*
	 * Find each TRs "inner div" (div within first cell). This div controls each
	 * TRs height. Returns the max pixel height.
	 */
  ...
  /*
	 * Find each TRs "inner div" and sets all of their heights to the same
	 * value.
	 */
  ...
  /*
	 * Triggered when the current node has been shown (either freshly rendered
	 * or re-shown) when it had previously been unrendered or hidden. `shown`
	 * does not bubble up the hierarchy.
	 */
  ...
  /*
	 * Triggered when the current node has been hidden (either temporarily or
	 * permanently) when it had previously been shown. `hidden` does not bubble
	 * up the hierarchy.
	 */
  ...
  /*
	 * Just like `shown`, but only triggered for nodes that are actual rows.
  ...
  /*
	 * Just like `hidden`, but only triggered for nodes that are actual rows.
	 * Bubbles up the hierarchy.
	 */
  ...
  /*
	 * Triggered when the current node has been added to the hierarchy. `added`
	 * does not bubble up.
	 */
  ...
  /*
	 * Triggered when the current node has been removed from the hierarchy.
	 * `removed` does not bubble up.
	 */
  ...
  /*
	 * Just like `added`, but only triggered for nodes that are actual rows.
	 * Bubbles up the hierarchy.
	 */
  ...
  /*
	 * Just like `removed`, but only triggered for nodes that are actual rows.
	 * Bubbles up the hierarchy.
	 */
...
/*
 * An abstract node in a row-hierarchy tree that contains other nodes. Will have
 * some sort of rendered label indicating the grouping, up to the subclass for
...
RowGroup = (function(superClass) {
  ...
  /*
	 * Renders the content wrapper element that will be inserted into this row's
	 * TD cell
	 */
  ...
  /*
	 * Renders the text span element that will be inserted into this row's TD
	 * cell. Goes within the content element.
	 */
...
/*
 * A row grouping that renders as a single solid row that spans width-wise (like
 * a horizontal rule)
 */
...
HRowGroup = (function(superClass) {
  ...
  /*
	 * Renders this row's TR for the "spreadsheet" quadrant, the area with info
	 * about each resource
	 */
...
/*
 * A row grouping that renders as a tall multi-cell vertical span in the
 * "spreadsheet" area
 */
...
VRowGroup = (function(superClass) {
  ...
  /*
	 ...
	 * PRECONDITION: in the case of multiple group nesting, a child's
	 * renderRowspan() will be called before the parent's renderRowspan().
	 */
...
/*
 * A row that renders information about a particular resource, as well as it
 * events (handled by superclass)
 */
...
ResourceRow = (function(superClass) {
  ...
  ResourceRow.prototype.renderSpreadsheetContent = function(tr) {
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      ...
      var templateOperador = '<table>'
          +'<tr>'
          +'<td class="imgOperarioCont" style="width: 65px;">'+ resource.avatar+'</td>'
          +'<td style="border: 0; padding:2px;">'
          +'<div class="container-fluid">'
          	+'<div class="row">'
          		+'<small><strong class="operarioList-'+resource.id+'">'+resource.titulo+'</strong></small>'
          	+'</div><div class="row">'
          		+'<small><strong>Empleado: </strong>'+resource.nemploy+'</small>'
          	+'</div><div class="row">' 
          		+'<small><i class="fa fa-phone" aria-hidden="true" style="color: #1ca4fc;"></i> '+(resource.telefono ?resource.telefono : 'S/T')+'</small>'
          	+'</div><div class="row">'
          		+'&nbsp;&nbsp;&nbsp;<img class="cursorImg cursorEfect" src="'+finalphat+'/img/operarios/ic-eye.png" onclick="cambiaStatusOperario(\''+resource.titulo+'\',\''+resource.id+'\',\''+resource.statusoper+'\',\''+resource.idstatus+'\');" style="padding-top: 2px; height: 10px;">&nbsp;&nbsp;&nbsp;'
          		+'<img class="cursorImg cursorEfect" src="'+finalphat+'/img/operarios/ic-informacion.png" onclick="consultaOTOperario(\''+resource.id+'\', \''+resource.titulo+'\');" style="height: 15px;">&nbsp;&nbsp;&nbsp;'
          		+'<img class="cursorImg cursorEfect" src="'+finalphat+'/img/operarios/ic-poi.png" onclick="viewMapOperario(\''+resource.id+'\', \''+resource.titulo+'\',\''+resource.latitud+'\', \''+resource.longitud+'\',\''+resource.statusoper+'\');" style="height: 15px;">'
          	+'</div>'
          +'</div></td>'
          +'<td class="allOperario" style="background-color:'+ resource.statusoper +'">'+resource.numots+'</td>'
          +'</tr>'
          +'</table>'
          +'<input type="hidden" value="'+resource.idstatus+'" id="bloqueoOper-'+resource.id+'">';
      contentEl = $('<div class="fc-cell-content" >' + (colSpec.isMain ? this.renderGutterHtml() : '') + '<div class="fc-cell-text">' +  templateOperador + '</div>' + '</div>');
// contentEl = $('<div class="fc-cell-content">' + (colSpec.isMain ?
// this.renderGutterHtml() : '') + '<span class="fc-cell-text">' + (text ?
// htmlEscape(text) : '&nbsp;') + '</span>' + '</div>');
  ...
  /*
	 * Renders the HTML responsible for the subrow expander area, as well as the
	 * space before it (used to align expanders of similar depths)
	 */
...
/*
 * This decryption is not meant to be bulletproof. Just a way to remind about an
 * upgrade.
 */
